...
<div class="bg-white rounded-lg shadow">
    ...
    <form method="POST" action="{{ route('articles.update', $article) }}" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PUT')
                    ...
                    <div class="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center">
                        ...
                        @if($article->thumbnail)
                            <div id="current-thumbnail">
                                <img src="{{ asset('storage/' . $article->thumbnail) }}" alt="Current thumbnail" class="w-full h-32 object-cover rounded-lg mb-2">
                                <p class="text-sm text-gray-600 mb-2">Thumbnail saat ini</p>
                                <button type="button" onclick="showUpload()" class="text-blue-600 text-sm hover:underline">Ganti thumbnail</button>
                            </div>
                        @endif
